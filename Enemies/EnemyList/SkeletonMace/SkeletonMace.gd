...
#==== Comments ====#
# This is a bulkier version of the skeleton. It has more hp and after receiving
# some damage, will ignore the "hurt" state and instantly retaliate.
...
#==== Constants ====# 
const _loot_type = 1 # 0:low, 1:mid, 2:high
...
func _ready():
	...
	_hurtbox_shape = $StandardHitbox/HurtBox/HurtBoxShape
	...
	_attack = preload("res://Enemies/EnemyList/SkeletonMace/SkeletonMaceAttack.gd")
	change_direction()
...
func handle_hurt_recovery():
	var is_player_in_range = _physics_manager.handle_player_range()
	if is_player_in_range[0]: # is_in_attack_range 
			_state_manager.set_state_attack()
	else:
