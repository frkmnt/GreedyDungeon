...
func randomize_chests_in_room(room, room_number):
	randomize()
	var random_chest_index = floor(rand_range(0, 201))
	if random_chest_index <= 500:
		var chest_instance = _CHEST_PREFAB.instance()
		chest_instance.initialize(_DOUBLE_JUMP_PREFAB.instance())
...
func randomize_money_in_room(room, money_odds):
	for money_prefab in room._money_container.get_children():
		randomize()
		var random_money_index = floor(rand_range(0, 201))
